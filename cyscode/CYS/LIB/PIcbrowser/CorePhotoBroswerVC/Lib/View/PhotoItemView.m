...
-(void)dataPrepare{
    ...
    if(isNetWorkShow){//网络请求
        ...
        if(image == nil) return;
        [self.photoImageView sd_setImageWithURL:[NSURL URLWithString:_photoModel.image_HD_U] placeholderImage:image options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
//        }];
    ...
    self.photoImageView.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
