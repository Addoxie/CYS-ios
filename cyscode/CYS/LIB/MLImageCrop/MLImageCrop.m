...
- (void)viewDidLoad
{
    ...
    //绘制俩button
    self.cancelButton = [self acquireCustomButtonWithTitle:@"取消" andAction:@selector(onCancel:)];
    [self.cancelButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.view addSubview:self.cancelButton];

    
     self.confirmButton  = [self acquireCustomButtonWithTitle:@"选取" andAction:@selector(onConfirm:)];
    [self.confirmButton setTitleColor:[UIColor colorWithRed:255/255.0 green:136/255.0 blue:112/255.0 alpha:1] forState:UIControlStateNormal];
...
#define kButtonWidth 50
    self.cancelButton.frame = CGRectMake(0, CGRectGetMinY(self.buttonBackgroundView.frame), ScreenWidth/2, kButtonViewHeight);
    self.confirmButton.frame = CGRectMake(ScreenWidth/2, CGRectGetMinY(self.buttonBackgroundView.frame), ScreenWidth/2, kButtonViewHeight);
