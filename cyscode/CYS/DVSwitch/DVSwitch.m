...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i < [self.labels count]; i++) {
        ...
        if (self.isneedimage) {
            ...
            NSArray *imagenamearr=@[@"tmp",@"HAHA",@"KAKA"];
            NSString *imagename=[imagenamearr objectAtIndex:i];
        
            if (ScreenWidth>=320&&ScreenWidth<375) {
                UIImageView *imagev=[[UIImageView alloc]initWithFrame:CGRectMake(label.frame.origin.x+5, 10, 15, 15)];
                imagev.image=[UIImage imageNamed:imagename];
                [self.backgroundView addSubview:imagev];
                
            }else if (ScreenWidth>=375&&ScreenWidth<414){
           
                UIImageView *imagev=[[UIImageView alloc]initWithFrame:CGRectMake(label.frame.origin.x+10, 10, 15, 15)];
                imagev.image=[UIImage imageNamed:imagename];
                [self.backgroundView addSubview:imagev];
                
            }else{
                
                UIImageView *imagev=[[UIImageView alloc]initWithFrame:CGRectMake(label.frame.origin.x+14, 10, 15, 15)];
                imagev.image=[UIImage imageNamed:imagename];
                [self.backgroundView addSubview:imagev];
            }
    ...
    for (int j = 0; j < [self.onTopLabels count]; j++) {
        ...
        if (self.isneedimage) {
            ...
            if (ScreenWidth>=320&&ScreenWidth<375) {
                selectedimagev=[[UIImageView alloc]initWithFrame:CGRectMake(5, 10, 15, 15)];
                selectedimagev.image=[UIImage imageNamed:imagename];
                [self.sliderView addSubview:selectedimagev];
                
            }else if (ScreenWidth>=375&&ScreenWidth<414){
                
                selectedimagev=[[UIImageView alloc]initWithFrame:CGRectMake(10, 10, 15, 15)];
                selectedimagev.image=[UIImage imageNamed:imagename];
                [self.sliderView addSubview:selectedimagev];
                
            }else{
                
                selectedimagev=[[UIImageView alloc]initWithFrame:CGRectMake(14, 10, 15, 15)];
                selectedimagev.image=[UIImage imageNamed:imagename];
                [self.sliderView addSubview:selectedimagev];
            }
...
//            UIImageView *imagev=[[UIImageView alloc]initWithFrame:CGRectMake(13, 10, 15, 15)];
//            imagev.image=[UIImage imageNamed:@"tmp"];
//            [self.sliderView addSubview:imagev];
...
- (void)animateChangeToIndex:(NSUInteger)selectedIndex
{
    ...
    NSArray *imagenamearr=@[@"tmp",@"HAHA",@"KAKA"];
    NSString *imagename=[imagenamearr objectAtIndex:selectedIndex];
