...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([correspondingIndexPath subRow] == 0)
    {
        ...
        [expandableCell addSubview:button];
...
- (void)canclebuttonaction:(CutomUIButton *)button{
    ...
    indexPath.subRow=button.subrow;
   ...
   // NSDictionary *dic=(NSDictionary *)self.contents[button.section][button.row][button.subrow];
//    
...
- (void)buttonaction:(CutomUIButton *)button{
   ...
   // commentLongRecognizer=button;
    ...
    //  openedpath=[NSIndexPath indexPathForRow:button.row inSection:button.section];
    //    openedpath.subRow=button.subrow;
    //    
    //    [self.contents[button.section][button.row] removeObjectAtIndex: button.subrow];
    // 
...
-(void)deletecell:(NSTimer *)cutimer{
    ...
    if (timercount<1) {
        ...
        timercount=0;
        [timer invalidate];
        //commentLongRecognizer.alpha=0.1;'
        ...
        [self.SKSTableViewDelegate openedit:commentmodel];
...
//        
//        UIAlertView *alertview=[[UIAlertView alloc]initWithTitle:@"提示" message:@"fuck？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"删除", nil];
//        [alertview show];
    }
