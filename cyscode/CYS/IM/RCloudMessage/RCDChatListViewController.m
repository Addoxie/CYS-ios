...
-(id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
       ...
       // [self setConversationAvatarStyle:RCUserAvatarCycle];
        [[RCIMClient sharedRCIMClient]getConversation:ConversationType_GROUP targetId:nil];
    }
    return self;
}
-(id)initWithDisplayConversationTypes:(NSArray *)displayConversationTypeArray collectionConversationType:(NSArray *)collectionConversationTypeArray{
    ...
    if (self) {
//        NSLog(@"%@",displayConversationTypeArray);
//        
//        NSLog(@"%@",displayConversationTypeArray);
        
//        if (self.isgroup) {
//             [self.conversationListTableView registerClass:[RCDChatListCell class] forCellReuseIdentifier:@"chatlistcell"];
//        }
...
- (void)viewDidLoad {
  ...
  //  self.conversationListTableView.tableFooterView = [UIView new];
    ...
    if (self.isgroup) {
      ...
      //  self.conversationListTableView.tableHeaderView =headerview;
       ...
       // self.conversationListTableView.tableHeaderView =headerview;
        UIButton *button=[UIButton buttonWithType:UIButtonTypeCustom];
        button.frame=headerview.frame;
       // [button setTitle:@"我的患者" forState:UIControlStateNormal];
        [button setTitleColor:KBlackColor forState:UIControlStateNormal];
        [button addTarget:self action:@selector(mypatientaction) forControlEvents:UIControlEventTouchUpInside];
        [headerview addSubview:button];
        UILabel *itemlabel=[[UILabel alloc]initWithFrame:CGRectMake(15, 0, 90, 50)];
        itemlabel.text=@"我的患者";
        itemlabel.textColor=KBlackColor;
    ...
    [self.conversationListTableView registerClass:[RCDChatListCell class] forCellReuseIdentifier:@"chatcell"];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.isgroup&&self.isneedmorebtn) {
        UIButton *rightBtn = [[UIButton alloc] initWithFrame:CGRectMake(ScreenWidth-40, 5, 30, 30)];
        ...
        [rightBtn setTitleColor:KlightOrangeColor forState:UIControlStateNormal];
...
-(void)onSelectedTableRow:(RCConversationModelType)conversationModelType conversationModel:(RCConversationModel *)model atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_isClick) {
        ...
        if (model.conversationModelType == RC_CONVERSATION_MODEL_TYPE_PUBLIC_SERVICE) {
            ...
            NSLog(@"%zd",model.conversationType);
            _conversationVC.targetId = @"dongbin";
            _conversationVC.userName = @"yangqing";
        ...
        if (conversationModelType == RC_CONVERSATION_MODEL_TYPE_NORMAL) {
            ...
            if (self.isgroup) {
                ...
                [IMDataService getListingIMInfofortargetIdWithtargetId:model.targetId block:^(id userdic) {
                    [hud dismiss];
                    NSLog(@"%@",userdic);
                    NSLog(@"%@",userdic);
                    ...
                    conversationPageVC.title=model.conversationTitle;
                    conversationPageVC.patientd=[userdic objectForKey:@"id"];
...
//    
//    self.tabBarController.navigationController.tabBarController.navigationController.navigationBar.hidden=NO;
//    [self.tabBarController.navigationController.tabBarController.navigationController pushViewController:adddoctorteamVC animated:YES];
...
//    
////    NSLog(@"%@",model.conversationType);
////    NSLog(@"%@",model.conversationType);
//    
//    RCDChatListCell *cell;
//    if (self.isgroup&&model.conversationType==ConversationType_GROUP) {
//        cell = [[RCDChatListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"chatcell"];
//        cell.lblDetail.text =[model.jsonDict objectForKey:@"content"];
//        
//        [cell.ivAva sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",@""]] placeholderImage:[UIImage imageNamed:kPlaceholderimage]];
...
//
//    }else if (self.isgroup==NO&&model.conversationType==ConversationType_PRIVATE){
//        
//       cell = [[RCDChatListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"chatcell"];
//        cell.model=model;
//        [cell setDataModel:model];
//        cell.lblDetail.text =[NSString stringWithFormat:@"%@",[model.jsonDict objectForKey:@"content"]];
//        //[cell.ivAva sd_setImageWithURL:[NSURL URLWithString:portraitUri] placeholderImage:[UIImage imageNamed:@"system_notice"]];
//        [cell.ivAva sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",@""]] placeholderImage:[UIImage imageNamed:kPlaceholderimage]];
//        cell.labelTime.text = [self ConvertMessageTime:model.sentTime / 1000];
//
...
//        BOOL isinList=NO;
//        
//        for (NSString *groupid in self.grouparr) {
//            if([message.targetId rangeOfString:groupid].location !=NSNotFound){
//              
//                isinList=YES;
//            }
//        }
