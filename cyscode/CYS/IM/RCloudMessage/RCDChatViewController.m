...
@interface RCDChatViewController () <UIActionSheetDelegate, RCRealTimeLocationObserver, RealTimeLocationStatusViewDelegate, UIAlertViewDelegate, RCMessageCellDelegate>{
    UIView *headerview;
    NSMutableDictionary *_serviceDataDic;
        ...
        [OrderDataService getAllServiceConfigsblock:^(id respdic) {
            ...
            [OrderDataService getServiceDetailWithServicetype:@"" block:^(id respdic0) {
                ...
                detaildic=[[NSDictionary alloc]initWithDictionary:respdic0];
                    ...
                    NSDictionary *dic1=[detaildic objectForKey:[NSString stringWithFormat:@"%zd",0]];
                
                if ([[NSString stringWithFormat:@"%@",[dic1 objectForKey:@"status"]]isEqualToString:@"1"]) {
                    return ;
                    
                } else{
                    NSDictionary *dic2=[detaildic objectForKey:[NSString stringWithFormat:@"%zd",2]];
                    
                    if ([[NSString stringWithFormat:@"%@",[dic2 objectForKey:@"status"]]isEqualToString:@"1"]) {
...
-(void)configheaderview{
    [headerview removeFromSuperview];
    headerview=[[UIView alloc]initWithFrame:CGRectMake(10, 120-64, ScreenWidth-20, 55)];
    headerview.backgroundColor=kimiColor(255, 239, 188, 1.0);
    
    UIImageView *imagev=[[UIImageView alloc]init];
    //=[[UIImageView alloc]init];
    imagev.frame=CGRectMake(5, 55/2.0-30/2.0, 30, 30);
    imagev.layer.cornerRadius =5.0;
    imagev.image=[UIImage imageNamed:@"notice"];
   // [imagev sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",KGetDefaultstr(@"icon-url")]] placeholderImage:[UIImage imageNamed:kPlaceholderimage]];
    ...
    UILabel *titlelabel=[[UILabel alloc]initWithFrame:CGRectMake(45, 10, ScreenWidth-20-45, 35)];
    titlelabel.text=@"暂未开通权限，无法进行对话，马上设置";
...
-(void)reloadindex:(NSNotification *)sender{
    ...
    if (![KGetDefaultstr(@"userstatus") isEqualToString:@"AUDIT_PASSED"]) {
        ...
        headerview=[[UIView alloc]initWithFrame:CGRectMake(10, 120, ScreenWidth-20, 55)];
        headerview.backgroundColor=kimiColor(255, 239, 188, 1.0);
...
-(void)hahamessage{
                ...
                if (discussion != nil && discussion.memberIdList.count>0) {
                    if ([discussion.memberIdList containsObject:[RCIMClient sharedRCIMClient].currentUserInfo.userId]) {
                        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]
                                                                  initWithImage:[UIImage imageNamed:@"setting_up"]
    ...
    //  [self.navigationController.navigationBar setTintColor:KlightOrangeColor];
    //    [RCIM sharedRCIM].userInfoDataSource = self;
    //    [RCIM sharedRCIM].groupInfoDataSource = self;
    //self.emojiBoardView.userInteractionEnabled=NO;
   // self.pluginBoardView.userInteractionEnabled=NO;
   //  self.inputView.hidden=YES;
#pragma 隐藏输入组件
//    self.chatSessionInputBarControl.hidden = YES;
//    UIView *bgv=[[UIView alloc]initWithFrame:CGRectMake(0, ScreenHeight-49, ScreenWidth, 49)];
...
- (void)notifyUpdateUnreadMessageCount {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    backImg.frame = CGRectMake(-10, 2, 10, 22);
    [backBtn addSubview:backImg];
    UILabel *backText = [[UILabel alloc] initWithFrame:CGRectMake(6, 3, 85, 22)];
...
//    RichMessageViewController *sentRMvc=[[RichMessageViewController alloc]init];
//    
//    [self.navigationController pushViewController:sentRMvc animated:YES];
...
//
//    
//      [self sendMessage:richmessage pushContent:@"给你发张图片"];
//    
    
    
    
    NSLog(@"%@",userId);
    if ([userId substringToIndex:1]) {
         NSLog(@"%@",[userId substringToIndex:1]);
    }else{
        [self.delegate chatViewDealtdataMethodname:nil stringval:nil];
