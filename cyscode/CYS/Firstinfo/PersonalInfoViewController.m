...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if([urlComps count] && [[urlComps objectAtIndex:0] isEqualToString:@"cysnative"])
    {
        ...
        if (1 == [arrFucnameAndParameter count])
        {
            ...
            }else if([funcStr isEqualToString:@"showbarcode"]){
            ...
            }else if([funcStr isEqualToString:@"personalPictureAction"]){
                _isPersonal=YES;
                [self selectPictureAction];
            ...
            }else if([funcStr isEqualToString:@"localiosSavePersonalSuccess"]){
                [hud dismiss];
    ...
    }else if([url rangeOfString:@"speciality"].location !=NSNotFound){
    ...
    }else if([url rangeOfString:@"intro"].location !=NSNotFound){
...
-(void)SpecialDealtdataMethodname:(NSString *)methodname stringval:(NSString *)stringval{
     [webview stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"%@('%@');",@"changeExpertise",stringval]];
    //[webview stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"%@('%@');",@"changeHospital",stringval]];
}
-(void)DescriptionDealtdataMethodname:(NSString *)methodname stringval:(NSString *)stringval{
      [webview stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"%@('%@');",@"changeIntro",stringval]];
}
...
didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    imageCrop.ratioOfWidthAndHeight = 200/200;
  ...
  //  [hud dismissAfterDelay:15];
    ...
    [[BaseDataService shareInstance]PostNetWorkDataWithURLStr:@"private/generalinfo/file" whitdic:dic theReturnBlock:^(id respdic) {
        ...
        if (_isPersonal) {
             ...
             [webview stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"setPersonalInformationHeader('%@');",[[[respdic objectAtIndex:1] objectForKey:@"result"] objectForKey:@"resource_url"]]];
        } else {
