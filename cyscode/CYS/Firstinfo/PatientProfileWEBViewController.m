//
//  PatientProfileWEBViewController.m
//  CYS
//
...
@interface PatientProfileWEBViewController ()
{
...
@implementation PatientProfileWEBViewController
...
-(void)loadprofileurl:(NSString *)profileurlstr{
    NSLog(@"%@",profileurlstr);
    NSURL *myurl=[NSURL URLWithString:profileurlstr];
    NSURLRequest *request=[NSURLRequest requestWithURL:myurl];
    hud=[[JGProgressHUD alloc]initWithStyle:JGProgressHUDStyleDark];
    [hud showInView:self.view];
    [hud dismissAfterDelay:25];
    [webview loadRequest:request];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if([url rangeOfString:@"healthData.html"].location !=NSNotFound)//
    {
        
       //
        PatientHealthDataViewController *VC=[[PatientHealthDataViewController alloc]init];
       // VC.delegate=self;
        VC.methodname=@"";
        VC.urslstr=url;
    ...
    }else if([url rangeOfString:@"case.html"].location !=NSNotFound)//
    {
        PatientHistoryViewController *VC=[[PatientHistoryViewController alloc]init];
        // VC.delegate=self;
        VC.methodname=@"";
        VC.urslstr=url;
        [self.navigationController pushViewController:VC animated:YES];
        ...
        return NO;
    }else if([url rangeOfString:@"label.html"].location !=NSNotFound)//
    {
    ...
    if([urlComps count] && [[urlComps objectAtIndex:0] isEqualToString:@"cysnative"])
    {
        ...
        if (1 == [arrFucnameAndParameter count])
        {
            ...
            // 没有参数
            if([funcStr isEqualToString:@"toPatientTags"])
            {
                NSLog(@"asd");
                /*调用本地函数*/
                //   [self testFunc];
            ...
            }else if([funcStr isEqualToString:@"returnToPatientList"]){
                ...
                [[PublicTools shareInstance]creareNotificationUIView:@"已经和该患者解除关系"];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [webview reload];
   //self.view.frame=CGRectMake(0.0f,0,ScreenWidth,ScreenHeight);
   // [hud dismiss];
